"""
parse_trackman_csv.py
=====================

This module contains a simple parser for TrackMan CSV export files.  It
converts each row of the CSV into a dictionary with normalised keys and
attempts to coerce numeric values to floats.  Each shot is assigned a
unique identifier.  The parser is designed to be robust against missing
columns and varying capitalisation by using ``dict.get`` with default
values.

The ``parse_trackman_csv`` function takes a file path and returns a list
of shot dictionaries.  A helper function ``try_float`` attempts to
convert strings to floats, returning ``None`` when the conversion fails.
"""
...
def parse_trackman_csv(file_path: str) -> List[Dict[str, Any]]:
    """Parse a TrackMan CSV file into a list of shot dictionaries."""
